              ...
              <Box sx={{ display: "flex", flexDirection: "column", flex: 1 }}>
                <CardContent>
                  ...
                  <Typography variant="body2" color="textSecondary">
                    items: {order.items && order.items.map((item: IMenu, index: number)=> `${item.name}${index==order.items.length-1 ?'':", "}`)}
                  </Typography>
                  <Typography variant="body2" color="textSecondary">
                    price: {order.items && order.items.reduce((acc: number, curr: IMenu)=> {
